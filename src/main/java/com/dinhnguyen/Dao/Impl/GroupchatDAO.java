...
public class GroupchatDAO extends AbstractDAO<GroupChat> implements IGroupchatDAO {
	...
	public void Update(GroupChat nhomChat) {
		if (nhomChat.getType().equals("admin")) {
			StringBuilder sql = new StringBuilder("Update nhomchat set ");
			if (nhomChat.getTennhom() != null) {
				sql.append(" tennhom = '" + nhomChat.getTennhom() + "' ,");
			}
			sql.append(" trangthai = " + nhomChat.getTrangthai());
			sql.append(" where id_nhomchat =" + nhomChat.getId_nhomchat());
			update(sql.toString());
			...
			if (nhomChat.getId_nguoitao() == null) {
				...
				update(sql.toString(), nhomChat.getTennhom(), nhomChat.getImage(), nhomChat.getTrangthai(),
						nhomChat.getId_nhomchat());
	...
	public List<GroupChat> findallNhomchatbyiduser(Long id_user) {
		StringBuilder sql = new StringBuilder(
				"select nc.id_nhomchat , nc.tennhom, nc.id_nguoitao , nc.image , nc.trangthai from nhomchat as nc , ctnhomchat as ct ");
		sql.append(
				" where nc.id_nhomchat = ct.id_nhomchat and nc.trangthai = 1 and ct.trangthai = 1 and ct.id_nguoidung = "
						+ id_user);
		try {
